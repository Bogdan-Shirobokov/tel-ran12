...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < sbor.length; i++) {
            System.out.println("[" + (i + 1) + "]" + sbor[i].toString() + "[" + (i + 1) + "]=" + sbor[i].srawgod(book1));
            System.out.println("================");
        }
        System.out.println();
        System.out.println("================");
    ...
    public static void print2 (Book1[]sbor){
        for (int i = 0; i < sbor.length; i++)
            System.out.println(sbor[i].toString());
    }


    public static void printRawGod(Book1[] book1s) {

        for (int i = 0; i < book1s.length; i++) {
            for (int j=0; j<book1s.length;j++)
                if (book1s[i].jaerOfPublik == book1s[j].jaerOfPublik) {
                    System.out.println(book1s[i].nameTheBook+" - ["+book1s[i].jaerOfPublik+"]");
